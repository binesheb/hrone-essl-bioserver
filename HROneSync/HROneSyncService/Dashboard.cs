...
public static class DashboardServer
{
    ...
    private static async Task SetCheckpoint(HttpListenerContext context)
    {
        ...
        try
        {
            ...
            await WriteJson(context, new { success = true, oldValue = old, newValue = request.LastProcessedDeviceLogId, message = "Checkpoint updated." }, 200);
        }
    ...
    private static async Task DateSync(HttpListenerContext context)
    {
        ...
        await WriteJson(context, new { success = true, found = true, date = date.ToString("yyyy-MM-dd"), firstPunch = new { deviceLogId = firstId, employeeCode = employee, logDate, deviceId = device }, recommendedCheckpoint = checkpoint, message = checkpoint == firstId - 1 ? "First punch found. The recommended checkpoint is the immediately preceding DeviceLogId." : "First punch found. The recommended checkpoint is the latest DeviceLogId before this punch." }, 200);
    }
...
<style>
:root{--bg:#0b1020;--p:#131a2b;--p2:#101729;--b:#26324b;--t:#edf2ff;--m:#93a1bd;--g:#35d07f;--a:#ffbe55;--r:#ff647c;--bl:#6ea8fe}*{box-sizing:border-box}body{margin:0;font-family:Segoe UI,Arial;background:var(--bg);color:var(--t)}header{padding:18px 30px;border-bottom:1px solid var(--b);display:flex;justify-content:space-between;gap:20px;align-items:center;position:sticky;top:0;background:rgba(11,16,32,.97);z-index:5}.brand strong{font-size:18px}.actions{display:flex;flex-direction:column;align-items:flex-end;gap:4px}.refresh{display:flex;align-items:center;gap:10px}.wrap{max-width:1500px;margin:auto;padding:20px 24px 40px}.grid{display:grid;gap:14px}.cards{grid-template-columns:repeat(4,minmax(0,1fr))}.two{grid-template-columns:1.2fr .8fr}.panel{background:var(--p);border:1px solid var(--b);border-radius:14px;padding:16px;margin-bottom:14px}.panel h3{margin:0 0 12px;font-size:15px}.label{font-size:11px;color:var(--m);text-transform:uppercase;letter-spacing:.05em}.value{font-size:28px;font-weight:700;margin-top:5px}.muted{color:var(--m)}button{border:0;border-radius:8px;padding:9px 13px;font-weight:700;cursor:pointer;background:var(--bl);color:#07101f}button.danger{background:var(--r);color:#fff}button.green{background:var(--g);color:#07101f}button.yellow{background:var(--a);color:#07101f}button.secondary{background:#24314d;color:var(--t)}button:disabled{opacity:.55;cursor:not-allowed}input{background:#0b1020;color:var(--t);border:1px solid var(--b);border-radius:8px;padding:9px;width:100%;margin:6px 0}.msg{margin-top:8px;color:var(--m);font-size:13px}.small{font-size:12px;color:var(--m)}.errorbox{background:#3a1720;border:1px solid var(--r);padding:12px;border-radius:10px;margin-bottom:14px}.health{display:flex;gap:9px;align-items:center;flex-wrap:wrap}.dot{width:9px;height:9px;border-radius:50%;display:inline-block;background:var(--a)}.dot.ok{background:var(--g)}.dot.bad{background:var(--r)}.row{display:flex;gap:9px;flex-wrap:wrap;align-items:end}.field{flex:1;min-width:170px}.success{color:var(--g)}.warn{color:var(--a)}.servicegrid{display:grid;grid-template-columns:1fr auto;gap:14px;align-items:center}.serviceactions{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end}.metricline{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--b)}.metricline:last-child{border-bottom:0}table{width:100%;border-collapse:collapse}th,td{padding:9px;border-bottom:1px solid var(--b);text-align:left;font-size:13px}th{color:var(--m);font-weight:600}.status{font-weight:700}.status.uploaded{color:var(--g)}.status.pending{color:var(--a)}.sectionhead{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:10px}.sectionhead h3{margin:0}.pill{padding:4px 8px;border-radius:20px;background:#24314d;color:var(--m);font-size:11px}@media(max-width:900px){.cards{grid-template-columns:repeat(2,minmax(0,1fr))}.two{grid-template-columns:1fr}.servicegrid{grid-template-columns:1fr}.serviceactions{justify-content:flex-start}}@media(max-width:560px){header{padding:14px 16px;align-items:flex-start}.actions{align-items:flex-end}.wrap{padding:14px}.cards{grid-template-columns:1fr 1fr}.refresh button{padding:8px 10px}.value{font-size:23px}}
</style></head><body><header><div class="brand"><strong>HROne ESSL Biometric Sync</strong><div class="small">Live synchronization control center</div></div><div class="actions"><div class="refresh"><button id="refresh">Refresh Now</button></div><div class="small" id="lastRefresh">Last refresh: —</div><div id="health" class="health"><span class="dot"></span><span>Checking...</span></div></div></header>
...
<div class="grid two"><section class="panel"><div class="sectionhead"><h3>Synchronization</h3><span class="pill">Live</span></div><div class="metricline"><span class="muted">LastProcessedDeviceLogId</span><strong id="checkpoint">—</strong></div><div class="metricline"><span class="muted">Latest DeviceLogId</span><strong id="latest">—</strong></div><div class="metricline"><span class="muted">Queue depth</span><strong id="queue">—</strong></div></section><section class="panel"><div class="sectionhead"><h3>Service</h3><span id="serviceBadge" class="pill">Checking</span></div><div class="metricline"><span class="muted">Database</span><strong id="dbStatus">—</strong></div><div class="metricline"><span class="muted">Windows Service</span><strong id="serviceStatus">—</strong></div></section></div>
<section class="panel"><h3>Sync from Date</h3><div class="small">Find the first punch on a selected date and set the checkpoint immediately before it.</div><div class="row"><div class="field"><label for="syncDate" class="small">Date</label><input id="syncDate" type="date"></div><button id="findDate" class="green">Find First Punch</button></div><div id="dateResult" class="msg"></div><div id="dateAction" style="margin-top:10px"></div></section>
<section class="panel"><h3>Manual Sync Point</h3><div class="small">Use this only when you intentionally need to overwrite the current checkpoint.</div><div class="row"><div class="field"><input id="checkpointInput" type="number" min="0" placeholder="LastProcessedDeviceLogId"></div><button id="setCheckpoint" class="yellow">Set Checkpoint</button></div><div id="checkpointMsg" class="msg"></div></section>
<section class="panel"><div class="sectionhead"><h3>Service Control</h3><span class="small">Current deployed executable</span></div><div class="servicegrid"><div><div class="label">Service</div><div id="servicePath" class="small">Location-aware service instance</div></div><div class="serviceactions"><button id="stop" class="danger">Stop Service</button><button id="restart" class="yellow">Restart Service</button><button id="update" class="green">Update Service</button></div></div><div id="serviceMsg" class="msg"></div></section>
<section class="panel"><div class="sectionhead"><h3>Recent Punches</h3><span id="recentCount" class="pill">0 records</span></div><div style="overflow:auto"><table><thead><tr><th>DeviceLogId</th><th>Employee</th><th>Time</th><th>Device</th><th>Status</th></tr></thead><tbody id="recent"></tbody></table></div></section>
</main>
<script>
...
function render(d,reason='Manual'){if(!d.ok){$('error').innerHTML='<div class="errorbox">'+esc(d.type||'Error')+': '+esc(d.error||'Dashboard error')+'</div>';return}$('error').innerHTML='';const s=d.summary;$('today').textContent=s.totalToday.toLocaleString();$('uploaded').textContent=s.uploadedToday.toLocaleString();$('pending').textContent=s.pending.toLocaleString();$('rate').textContent=s.successRate+'%';$('checkpoint').textContent=s.lastProcessedId.toLocaleString();$('latest').textContent=s.latestLogId.toLocaleString();$('queue').textContent=s.pending.toLocaleString();$('checkpointInput').value=s.lastProcessedId;$('lastRefresh').textContent='Last refresh: '+new Date(d.generatedAt).toLocaleString()+' • '+reason;$('recentCount').textContent=d.recent.length+' records';lastKnownLogId=s.latestLogId;$('recent').innerHTML=d.recent.map(x=>'<tr><td>'+x.id+'</td><td>'+esc(x.employee)+'</td><td>'+x.time+'</td><td>'+esc(x.machine)+'</td><td class="status '+(x.status==='Uploaded'?'uploaded':'pending')+'">'+x.status+'</td></tr>').join('')}
async function refresh(reason='Manual'){if(busy)return;busy=true;try{const d=await getJson('/api/data');render(d,reason);await health()}catch(e){$('error').innerHTML='<div class="errorbox">'+esc(e.message)+'</div>'}finally{busy=false}}
async function poll(){if(document.hidden)return;try{const s=await getJson('/api/punch-state');if(lastKnownLogId!==null&&s.latestLogId>lastKnownLogId)await refresh('New punch detected')}catch{} }
$('refresh').onclick=()=>refresh('Manual');
$('setCheckpoint').onclick=async()=>{const v=Number($('checkpointInput').value);if(!Number.isInteger(v)||v<0)return $('checkpointMsg').textContent='Enter a valid DeviceLogId.';if(!confirm('Set LastProcessedDeviceLogId to '+v+'?'))return;try{const j=await getJson('/api/checkpoint',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({lastProcessedDeviceLogId:v})});$('checkpointMsg').textContent='Updated from '+j.oldValue+' to '+j.newValue;await refresh('Checkpoint changed')}catch(e){$('checkpointMsg').textContent=e.message}};
$('findDate').onclick=async()=>{const date=$('syncDate').value;if(!date)return $('dateResult').textContent='Select a date first.';$('findDate').disabled=true;$('dateResult').textContent='Searching for the first punch...';$('dateAction').innerHTML='';try{const j=await getJson('/api/date-sync',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({date})});$('dateResult').innerHTML='<span class="success">First punch: '+j.firstPunch.logDate+' • Employee: '+esc(j.firstPunch.employeeCode)+' • DeviceLogId: '+j.firstPunch.deviceLogId+' • Device: '+esc(j.firstPunch.deviceId)+'</span><br>Recommended checkpoint: <strong>'+j.recommendedCheckpoint+'</strong>';$('dateAction').innerHTML='<button id="applyDate" class="yellow">Set Sync Point to '+j.recommendedCheckpoint+'</button>';$('applyDate').onclick=async()=>{if(!confirm('Set the sync point to '+j.recommendedCheckpoint+' so syncing begins with DeviceLogId '+j.firstPunch.deviceLogId+'?'))return;try{const r=await getJson('/api/checkpoint',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({lastProcessedDeviceLogId:j.recommendedCheckpoint})});$('dateAction').innerHTML='<span class="success">Sync point updated: '+r.newValue+'</span>';await refresh('Date sync point changed')}catch(e){$('dateAction').innerHTML='<span class="warn">'+esc(e.message)+'</span>'}}}catch(e){$('dateResult').innerHTML='<span class="warn">'+esc(e.message)+'</span>'}finally{$('findDate').disabled=false}};
async function service(action){if(action!=='update'&&!confirm('Are you sure you want to '+action+' the service?'))return;$('serviceMsg').textContent='Requesting '+action+'...';try{const j=await getJson('/api/service/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action})});$('serviceMsg').textContent=j.message||'Done';if(action==='update')$('servicePath').textContent=j.path||'Current executable updated';if(action==='restart')setTimeout(()=>health(),5000)}catch(e){$('serviceMsg').textContent=e.message}}
$('stop').onclick=()=>service('stop');$('restart').onclick=()=>service('restart');$('update').onclick=()=>service('update');
async function health(){try{const h=await getJson('/api/health');$('health').innerHTML='<span class="dot '+(h.ok?'ok':'bad')+'"></span><span>'+h.database+' • '+h.service+'</span>';$('dbStatus').textContent=h.database;$('serviceStatus').textContent=h.service;$('serviceBadge').textContent=h.service;$('serviceBadge').style.color=h.service==='Running'?'var(--g)':'var(--a)'}catch{$('health').innerHTML='<span class="dot bad"></span><span>Unavailable</span>';$('dbStatus').textContent='Unavailable';$('serviceStatus').textContent='Unavailable';$('serviceBadge').textContent='Unavailable'}}
function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
$('syncDate').value=new Date().toISOString().slice(0,10);refresh('Initial load');setInterval(poll,10000);setInterval(()=>{if(!document.hidden)refresh('Safe interval')},60000);setInterval(health,30000);
document.addEventListener('visibilitychange',()=>{if(!document.hidden)refresh('Tab active')});
</script></body></html>
""";
}