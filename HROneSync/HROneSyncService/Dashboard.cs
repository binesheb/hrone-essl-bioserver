...
public static class DashboardServer
{
    ...
    private static async Task SetCheckpoint(HttpListenerContext context)
    {
        ...
        {
            await WriteJson(context, new { error = "A valid LastProcessedDeviceLogId is required." }, 400);
            return;
        }
...
ELSE INSERT INTO dbo.HROneSyncState (LastProcessedDeviceLogId) VALUES (@Value);", connection, transaction))
    ...
    private static async Task ServiceControl(HttpListenerContext context)
    {
        ...
        {
            await WriteJson(context, new { error = "Action must be stop, restart, or update." }, 400);
            return;
        }
        ...
        if (!config.Success)
        {
            await WriteJson(context, new { success = false, action, message = config.Message }, 500);
            return;
        }
        ...
        {
            await WriteJson(context, new { error = "Service lifecycle control is not configured." }, 503);
            return;
        }
    ...
    private static (bool Success, string Message) ConfigureServiceToCurrentExecutable()
    {
        ...
        return result.ExitCode == 0
            ? (true, result.Output)
            : (false, $"Service configuration failed ({result.ExitCode}): {result.Output}");
    }
    ...
    private static async Task<object> GetDashboardData()
    {
        try
        {
            ...
            return new
            {
                ...
                health = new { database = true, service = GetServiceStatus() },
                summary = new
                {
                    totalToday = today,
                    uploadedToday = uploaded,
                    pending = Math.Max(0, latest - last),
                    successRate = today == 0 ? 100 : Math.Round(uploaded * 100.0 / today, 1),
                    lastProcessedId = last,
                    latestLogId = latest
                },
                recent = await GetRecent(c, last)
            };
        }
        catch (Exception ex)
        {
            return new
            {
                ...
                summary = new { totalToday = 0L, uploadedToday = 0L, pending = 0L, successRate = 0d, lastProcessedId = 0L, latestLogId = 0L },
    ...
    private static async Task<List<object>> GetRecent(SqlConnection c, long last)
    {
        ...
        while (await rd.ReadAsync())
        {
            ...
            {
                id = rd.GetInt64(0),
                employee = rd.IsDBNull(1) ? "-" : rd.GetString(1),
                time = rd.GetDateTime(2).ToString("yyyy-MM-dd HH:mm:ss"),
                machine = rd.GetValue(3).ToString(),
                status = rd.GetInt64(0) <= last ? "Uploaded" : "Pending"
            });
        }
        return r;
    }
