...
<section class="panel"><h3>Refresh</h3><button id="refresh">Refresh Now</button><div class="small" id="lastRefresh">Last refresh: —</div></section>
...
<section class="panel"><h3>Current Sync Point</h3><div class="row"><div class="field"><div class="label">LastProcessedDeviceLogId</div><div id="checkpoint" class="value">—</div></div></div><div class="small">You can still overwrite the checkpoint manually if required.</div><div class="row"><div class="field"><input id="checkpointInput" type="number" min="0" placeholder="DeviceLogId"></div><button id="setCheckpoint" class="yellow">Set Checkpoint</button></div><div id="checkpointMsg" class="msg"></div></section>
...
async function service(action){$('serviceMsg').textContent='Requesting '+action+'...';try{const j=await getJson('/api/service/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action})});$('serviceMsg').textContent=j.message||'Done';}catch(e){$('serviceMsg').textContent=e.message}}
