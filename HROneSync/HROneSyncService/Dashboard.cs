...
public static class DashboardServer
{
    ...
    private static object GetUpdateStatus()
    {
        try
        {
    ...
    {
        try { await using var c = await OpenConnection(); var last = await ScalarLong(c, "SELECT ISNULL(MAX(LastProcessedDeviceLogId),0) FROM dbo.HROneSyncState"); var latest = await ScalarLong(c, "SELECT ISNULL(MAX(DeviceLogId),0) FROM eBioServerNew.dbo.DeviceLogs"); var today = await ScalarLong(c, "SELECT COUNT_BIG(*) FROM eBioServerNew.dbo.DeviceLogs WHERE CAST(LogDate AS date)=CAST(GETDATE() AS date)"); var uploaded = await ScalarLong(c, "SELECT COUNT_BIG(*) FROM dbo.HROnePunchStatus WHERE Status='Uploaded' AND EXISTS (SELECT 1 FROM eBioServerNew.dbo.DeviceLogs dl WHERE dl.DeviceLogId=HROnePunchStatus.DeviceLogId AND CAST(dl.LogDate AS date)=CAST(GETDATE() AS date))"); var blockedPending = await ScalarLong(c, "SELECT COUNT_BIG(*) FROM dbo.HROnePunchStatus WHERE Status='Pending'"); var pending = Math.Max(0, latest - last); var ignored = await ScalarLong(c, "SELECT COUNT_BIG(*) FROM dbo.HROnePunchStatus WHERE Status='Ignored' AND EXISTS (SELECT 1 FROM eBioServerNew.dbo.DeviceLogs dl WHERE dl.DeviceLogId=HROnePunchStatus.DeviceLogId AND CAST(dl.LogDate AS date)=CAST(GETDATE() AS date))"); var pendingPunchOut = await ScalarLong(c, @"WITH LatestPunch AS (SELECT EmployeeCode, Direction, ROW_NUMBER() OVER (PARTITION BY EmployeeCode ORDER BY LogDate DESC, DeviceLogId DESC) AS rn FROM eBioServerNew.dbo.DeviceLogs WHERE CAST(LogDate AS date)=CAST(GETDATE() AS date) AND EmployeeCode IS NOT NULL AND LTRIM(RTRIM(EmployeeCode))<>'') SELECT COUNT_BIG(*) FROM LatestPunch WHERE rn=1 AND UPPER(LTRIM(RTRIM(ISNULL(Direction,''))))='IN'"); return new { ok = true, generatedAt = DateTime.Now, health = new { database = true, service = GetServiceStatus() }, summary = new { totalToday = today, uploadedToday = uploaded, ignoredToday = ignored, pending, blockedPending, pendingPunchOut, successRate = today == 0 ? 100 : Math.Round((uploaded + ignored) * 100.0 / today, 1), lastProcessedId = last, latestLogId = latest }, recent = await GetRecent(c, last) }; }
        catch (Exception ex) { return new { ok = false, generatedAt = DateTime.Now, error = ex.Message, type = ex.GetType().Name, summary = new { totalToday = 0L, uploadedToday = 0L, pending = 0L, pendingPunchOut = 0L, successRate = 0d, lastProcessedId = 0L, latestLogId = 0L }, recent = Array.Empty<object>() }; }
    }
    ...
    private static async Task<List<object>> GetRecent(SqlConnection c, long last)
    {
       ...
       COALESCE(ps.Status, CASE WHEN d.DeviceLogId<=@LastId THEN 'Uploaded' ELSE 'Pending' END) AS SyncStatus
...
LEFT JOIN dbo.HROnePunchStatus ps ON ps.DeviceLogId = d.DeviceLogId
ORDER BY d.DeviceLogId DESC";
        await using var cmd = new SqlCommand(sql, c) { CommandTimeout = 10 };
        cmd.Parameters.AddWithValue("@LastId", last);
        ...
        while (await rd.ReadAsync())
        {
            ...
            var status = rd.IsDBNull(6) ? "Pending" : rd.GetString(6);
            r.Add(new { id = rd.GetInt64(0), employee = name, employeeCode = code, time = rd.GetDateTime(3).ToString("yyyy-MM-dd HH:mm:ss"), machine = deviceName, status });
        }
        return r;
    }
...
function render(d,reason='Manual'){if(!d.ok){$('error').innerHTML='<div class="errorbox">'+esc(d.type||'Error')+': '+esc(d.error||'Dashboard error')+'</div>';return}$('error').innerHTML='';const s=d.summary;$('today').textContent=s.totalToday.toLocaleString();$('uploaded').textContent=s.uploadedToday.toLocaleString();$('ignoredTodayInline').textContent='(+'+(s.ignoredToday||0).toLocaleString()+')';$('pending').textContent=s.pending.toLocaleString();$('pendingInline').textContent=s.pending.toLocaleString();$('pendingPunchOut').textContent=s.pendingPunchOut.toLocaleString();$('rate').textContent=s.successRate+'%';$('checkpoint').textContent=s.lastProcessedId.toLocaleString();$('latest').textContent=s.latestLogId.toLocaleString();$('checkpointInput').value=s.lastProcessedId;$('lastRefresh').textContent='Last refresh: '+new Date(d.generatedAt).toLocaleString()+' • '+reason;lastKnownLogId=s.latestLogId;$('recent').innerHTML=d.recent.map(x=>'<tr><td>'+x.id+'</td><td>'+esc(x.employeeCode)+'</td><td><strong>'+esc(x.employee)+'</strong></td><td>'+x.time+'</td><td>'+esc(x.machine)+'</td><td><span class="badge '+(x.status==='Uploaded'?'success':x.status==='Pending'?'warn':'')+'">'+esc(x.status)+'</span></td></tr>').join('')}
