...
public static class DashboardServer
{
    ...
    private static async Task<List<object>> GetRecent(SqlConnection c, long last) { var r = new List<object>(); await using var cmd = new SqlCommand("SELECT TOP 25 DeviceLogId,EmployeeCode,LogDate,DeviceId FROM eBioServerNew.dbo.DeviceLogs ORDER BY DeviceLogId DESC", c) { CommandTimeout = 10 }; await using var rd = await cmd.ExecuteReaderAsync(); while (await rd.ReadAsync()) r.Add(new { id = rd.GetInt64(0), employee = rd.IsDBNull(1) ? "-" : rd.GetString(1), time = rd.GetDateTime(2).ToString("yyyy-MM-dd HH:mm:ss"), machine = rd.GetValue(3).ToString(), status = rd.GetInt64(0) <= last ? "Uploaded" : "Pending" }); return r; }
...
<section class="panel"><div class="sectionhead"><h3>Recent Punches</h3><span class="small">Latest 25 records</span></div><div class="tablewrap"><table><thead><tr><th>DeviceLogId</th><th>Employee</th><th>Time</th><th>Device</th><th>Status</th></tr></thead><tbody id="recent"></tbody></table></div></section></main>
<script>
...
function render(d,reason='Manual'){if(!d.ok){$('error').innerHTML='<div class="errorbox">'+esc(d.type||'Error')+': '+esc(d.error||'Dashboard error')+'</div>';return}$('error').innerHTML='';const s=d.summary;$('today').textContent=s.totalToday.toLocaleString();$('uploaded').textContent=s.uploadedToday.toLocaleString();$('pending').textContent=s.pending.toLocaleString();$('pendingInline').textContent=s.pending.toLocaleString();$('rate').textContent=s.successRate+'%';$('checkpoint').textContent=s.lastProcessedId.toLocaleString();$('latest').textContent=s.latestLogId.toLocaleString();$('checkpointInput').value=s.lastProcessedId;$('lastRefresh').textContent='Last refresh: '+new Date(d.generatedAt).toLocaleString()+' • '+reason;lastKnownLogId=s.latestLogId;$('recent').innerHTML=d.recent.map(x=>'<tr><td>'+x.id+'</td><td>'+esc(x.employee)+'</td><td>'+x.time+'</td><td>'+esc(x.machine)+'</td><td>'+x.status+'</td></tr>').join('')}
